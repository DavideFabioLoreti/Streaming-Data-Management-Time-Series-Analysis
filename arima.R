...
# 4️⃣ CREATE DUMMY VARIABLES FOR AUSTRALIAN HOLIDAYS
...
# 5️⃣ CREATE TIME SERIES (ADDING ANNUAL SEASONALITY)
...
# 6️⃣ OPTIMAL FOURIER PARAMETERS (INCREASED)
K_daily  <- 12  # Increased from 10 to 12 (24 regressors)
K_weekly <- 6   # Increased from 3 to 6 (8 regressors)
K_annual <- 1   # NEW: annual cycle regressors (4 total)
...
# 7️⃣ BUILD REGRESSOR MATRIX (INCLUDING ANNUAL CYCLE)
# Daily Fourier
...
# Combine all (24 + 8 + 4 + 4 = 40 regressors)
...
# 9️⃣ FUTURE REGRESSORS (INCLUDING ANNUAL)
...
# 1️⃣3️⃣ CHECK HOLIDAYS IN FORECAST
...
cat("\n==== CONFRONTO FORECAST vs STORIA GEN-FEB ====\n")

# 1️⃣ Estraiamo solo gennaio-febbraio dagli anni storici
df_obs_janfeb <- df_obs %>%
  filter(month(time) %in% c(1, 2)) %>%
  mutate(year = as.character(year(time)),
         type = "Storico")

# 2️⃣ Prepariamo la serie forecast
forecast_janfeb <- final_predictions %>%
  mutate(year = "Forecast",
         type = "Forecast")

# 3️⃣ Uniamo i dataset
df_plot <- bind_rows(df_obs_janfeb, forecast_janfeb)

# 4️⃣ Normalizziamo l’asse temporale (anno fittizio 2000)
df_plot <- df_plot %>%
