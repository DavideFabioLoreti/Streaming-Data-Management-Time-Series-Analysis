...
df_last2m = df.loc[start_2months:last_obs_time, 'value']

plt.figure(figsize=(16, 7))

# Observed series (last 2 months) in gray
plt.plot(df_last2m.index, df_last2m.values, label='Observed (last 2 months)', color='gray', linewidth=2, alpha=0.8)

# Vertical red dashed line at forecast start
plt.axvline(x=last_obs_time, color='red', linestyle='--', linewidth=2, alpha=0.9, label='Forecast start')

# LSTM forecast (future)
plt.plot(future_index, pred_lstm_future, label='LSTM forecast', linewidth=2)

plt.title('LSTM Forecast with Last 2 Months of Observed Series')
plt.xlabel('Time')
plt.ylabel('Value')
plt.legend(loc='best')
plt.grid(True, alpha=0.3)
